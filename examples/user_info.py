...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            df.to_json(user_file_path, orient='records', date_format='iso', force_ascii=False, default_handler=str, indent=2)

            # Save video_info as JSON
            videos_file_path = os.path.join(destination_folder, f'{channel}_videos.json')  # ファイル名は適宜変更してください
            df_videos = pd.DataFrame(videos)
            df_videos.to_json(videos_file_path, orient='records', date_format='iso', force_ascii=False, default_handler=str, indent=2)
            print("Done")
